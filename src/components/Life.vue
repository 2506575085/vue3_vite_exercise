...
function changeOne(select:Select, to:boolean) {
  if (to) {
    if (!selectBox.value[select.y]) {
      selectBox.value[select.y] = new Set([])
    }
    selectBox.value[select.y].add(select.x)
  } else {
    if (selectBox.value[select.y]) {
      selectBox.value[select.y].delete(select.x)
      if (selectBox.value[select.y].size == 0) {
        delete selectBox.value[select.y]
      }
    }
  }
}
